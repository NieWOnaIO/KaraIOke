...
async def get_songfile(song_id: str):
    ...
    if not os.path.exists(
        os.path.join(base_path, "vocals.mp3")
    ) or not os.path.exists(os.path.join(base_path, "no_vocals.mp3")):
        raise HTTPException(
            status_code=404, detail="Path does not exist"
        )
    ...
    zipf.write(
        os.path.join(base_path, "vocals.mp3"), arcname="vocals.mp3"
    )
    zipf.write(
        os.path.join(base_path, "no_vocals.mp3"),
        arcname="no_vocals.mp3",
    )
    zipf.write(
        os.path.join(zip_path, "metadata.json"), arcname="metadata.json"
    )
    zipf.close()
